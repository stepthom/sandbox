...
qplot(country, data=df, geom="bar", fill=status)  + theme(legend.position = "none")

qplot(sector, data=df, geom="bar", fill=status, xlab="sector") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme(legend.position = "none")

qplot(gender, data=df, geom="bar", fill=status) + theme(legend.position = "none")

qplot(nonpayment, data=df, geom="bar", fill=status) + theme(legend.position = "none")
...
A few sample rows of the final dataset.

```{r}
ids = c(1, 560, 709, 1111, 1278, 2345, 3984, 4000, 5000, 6000, 7000, 7566)
set.seed(124)
ids = floor(runif(20, min = 1, max = nrow(df_new)))
...
Below is the decision tree model that was learned from the training data, excluding the text (i.e., topic variables).
...
Below is the decision tree model that was learned from the training data, including all variables.
