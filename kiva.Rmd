...
Next, we used a techinque called Latent Dirichlet Allocation (LDA) to automatically extract high-level topics from the documents. We told LDA to extract the `12` most important topics; LDA will also tell us which topics are in which documents.
...
The figure below shows the number of documents that contain each topic, coloured by `status`.
