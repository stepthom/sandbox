...
Kiva's data science team is especially curious if the personal stories hold any predictive value. Text Analytics provides ways to understand and organize text data. Can Kiva's data science team use Text Analytics --- ways to understand and organize text data --- to leverage the personal stories to build a prediction system for loan applications, thereby reducing risk for lenders and Kiva?
...
Kiva provides public snapshots of all loan data on their website [http://build.kiva.org](http://build.kiva.org/). The data includes the following variables:
...
loan_amount Amount of the loan, in USD.
non_payment Who is liable if the loan defaults: the lender, or the partner?
en          Borrower's personal story.
status      Whether borrower defaulted or repaid loan.
---------   --------------
...
The overall goal of the data science team is to reduce risk to potential lenders, and to make sure loan resources are spent on borrowers with the highest likelihood of repaying. The team decides to build a prediction model (i.e., a _classifier_) that uses as much of the available historical data as possible in order to build an accurate model of which borrowers will repay, and which will default.

Most classifiers are able to accept numerical data (e.g., loan amount) and categorical data (e.g., gender, country) as inputs, but are not able to accept raw text data (e.g., en). Thus, the data science team must wrangle the text data to make it suitable for the classifier.

TODO: there are many ways to do this: individual words, key phrases, topics, etc. This team uses topics.

The first thing the data science team does is split the text into invidual words. For example, the sentence "She has dairy cows" will be split into four words: "She", "has", "dairy", and "cows".

The team then removes noise. This include fixing mispelled words, removing overly common words, and removing very rare words.

The then team inputs the cleaned words into an advanced topic model, called Latent Dirichlet Allocation (LDA) to automatically extract high-level topics from the text. For example, one personal story might contain the topics "dairy, cow, milk" and "wheat, farm, field", while another personal story might include the topics "clothing, garmet, shirt" and "children, child, kid." LDA will tell us which topics are in which personal stories. Note that the LDA automatically creates the topics from scratch, based on the input text; it does not have a list of common topics from which it chooses. As a result, the topics are specific to the text data in the given dataset. The extracted topics are show in Exhibit 2. Below is a sample of the data that will be used to train the classifiers:
...
Armed with all the numeric, categorical, and text (i.e., topics) variables in the The data science teams has lots of different classifier algorithms available. For this task, the team decides to use a decision tree classifier, because of its speed, accuracy, and interpritability. 

The team builds two decision tree classifiers. One is trained only on the numerical and categorical variables. The other is trained on the numerical, categorical, and text (i.e., topics) variables. This will help the team to determine if adding text to the classifiers is a good idea. Exhibit 3 shows the two classifiers and their performance.
