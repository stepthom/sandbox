...
```{r}
custom_stop_words = data.frame(word=c("br", "h4", "loan", "business", "font", "times", "span"))
...
a = c %>% cast_dtm(id, word, count)
a
str(a)
library(tm)
dtm2.sparse <- removeSparseTerms(a, 0.9995)
dtm2.sparse

rowTotals <- apply(dtm2.sparse , 1, sum) #Find the sum of words in each Document
which(rowTotals==0)
dtm.new   <- dtm2.sparse[rowTotals> 0, ] 
dtm.new

ap_lda <- LDA(dtm.new, k = 10, control = list(seed = 1234))
ap_lda
```
...
readr::write_csv(tidy(ap_lda, "beta"), "beta.csv")

readr::write_csv(tidy(ap_lda, "gamma"), "gamma.csv")

ap_documents <- tidy(ap_lda, matrix = "gamma")

ap_documents

ap_documents = ap_documents %>% spread(topic, gamma)
ap_documents$document = as.integer(ap_documents$document)


df  = df %>%
  left_join(ap_documents, by=c("id" = "document"))
df
```
...
```{r}
colnames(df)
form = as.formula(status ~ sector + country + gender + loan_amount + nonpayment + `1.y` + `2.y` + `3.y` + `4` + `5` + `6` + `7` + `8` + `9` + `10`)
